export module skinnedmeshdemo:app;
import std;
import shared;
import :sky;
import :renderstates;
...
namespace Skinned
{
	static constexpr auto MaxBoneTransforms = 96u;
...
public:
	...
	void DrawScene() override
	{
		...
		md3dImmediateContext->VSSetShader(
			mNormalMapSkinnedVertexShader.get(), nullptr, 0);
		md3dImmediateContext->PSSetShader(
			mNormalMapPixelShader.get(), nullptr, 0);
...
private:
	...
	void BuildShaders()
	{
		...
		HR(
			md3dDevice->CreateVertexShader(
				...
				shadowMapSkinnedVertexShaderBytecode->GetBufferSize(),
				nullptr,
		...
		HR(
			md3dDevice->CreateVertexShader(
				...
				normalDepthSkinnedVertexShaderBytecode->GetBufferSize(),
				nullptr,
		...
		CreateConstantBuffer(sizeof(Skinned::BoneConstants), mBoneTransformsCB);
		...
		D3D::ID3DBlob* normalMapSkinnedVertexShaderBytecode)
	{
		...
		auto skinnedDesc = std::array{
			...
			D3D11::D3D11_INPUT_ELEMENT_DESC{
				.SemanticName = "BONEINDICES",
				.SemanticIndex = 0,
				.Format = DXGI::DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UINT,
				.InputSlot = 0,
...
private:
	...
	std::unique_ptr<SkinnedModel> mCharacterModel;
	...
	ComPtr<D3D11::ID3D11InputLayout> mSkinnedInputLayout;
	...
	ComPtr<D3D11::ID3D11Buffer> mBoneTransformsCB;

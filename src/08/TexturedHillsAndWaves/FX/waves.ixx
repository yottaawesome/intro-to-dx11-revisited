export module texturedhillsandwaves:waves;
import std;
import shared;
...
public:
	...
	void Init(std::uint32_t m, std::uint32_t n, float dx, float dt, float speed, float damping)
	{
		...
		auto d = float{damping * dt + 2.0f};
		auto e = float{(speed * speed) * (dt * dt) / (dx * dx)};
		mK1 = (damping * dt - 2.0f) / d;
		mK2 = (4.0f - 8.0f * e) / d;
		mK3 = (2.0f * e) / d;

		// In case Init() called again.
		mPrevSolution = std::make_unique<DirectX::XMFLOAT3[]>(m * n);
		mCurrSolution = std::make_unique<DirectX::XMFLOAT3[]>(m * n);
		mNormals = std::make_unique<DirectX::XMFLOAT3[]>(m * n);
		mTangentX = std::make_unique<DirectX::XMFLOAT3[]>(m * n);

		// Generate grid vertices in system memory.

		auto halfWidth = float{(n - 1) * dx * 0.5f};
		auto halfDepth = float{(m - 1) * dx * 0.5f};
		for (auto i = 0u; i < m; ++i)
		{
			auto z = float{halfDepth - i * dx};
			for (auto j = 0u; j < n; ++j)
			{
				auto x = float{-halfWidth + j * dx};
	...
	void Update(float dt)
	{
		...
		if (t >= mTimeStep)
		{
			...
			for (auto i = 1u; i < mNumRows - 1; ++i)
			{
				for (auto j = 1u; j < mNumCols - 1; ++j)
				{
					auto l = float{mCurrSolution[i * mNumCols + j - 1].y};
					auto r = float{mCurrSolution[i * mNumCols + j + 1].y};
					auto t = float{mCurrSolution[(i - 1) * mNumCols + j].y};
					auto b = float{mCurrSolution[(i + 1) * mNumCols + j].y};
